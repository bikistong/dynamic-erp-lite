...
import { Plus, X, ChevronLeft, Users, Pencil, Search } from 'lucide-react';

interface Customer { id: string; name: string; email?: string; phone?: string; address?: string; city?: string; }
...
export default function CustomersPage() {
  ...
  const load = useCallback(async (q = '') => {
    setLoading(true);
    try {
      const res = await api.get('/sales/customers', { params: { limit: 100, search: q || undefined } });
  ...
  async function save() {
    setError('');
    if (!form.name) return setError('Nama customer wajib diisi.');
    setSaving(true);
    try {
      if (editing) await api.put(`/sales/customers/${editing.id}`, form);
      else await api.post('/sales/customers', form);
    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{customers.length} pelanggan terdaftar</p>
        </div>
          ...
          <tbody>
            ...
            {!loading && customers.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-10 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-600">Belum ada customer</p>
              </td></tr>
            )}
            {customers.map((c) => (
              <tr key={c.id} className="border-b border-gray-50 dark:border-gray-800/60 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                <td className="px-5 py-3.5 font-medium text-gray-900 dark:text-white">{c.name}</td>
                <td className="px-5 py-3.5 text-gray-500 dark:text-gray-400 text-xs">{c.email || '—'}</td>
                <td className="px-5 py-3.5 text-gray-500 dark:text-gray-400 text-xs">{c.phone || '—'}</td>
                <td className="px-5 py-3.5 text-gray-500 dark:text-gray-400 text-xs">{c.city || '—'}</td>
                <td className="px-5 py-3.5">
                  <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                    <Pencil size={13} />
                  </button>
                </td>
              </tr>
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
              <h3 className="font-semibold text-gray-900 dark:text-white">{editing ? 'Edit Customer' : 'Tambah Customer'}</h3>
            ...
            <div className="px-6 py-5 space-y-4">
              {[
                { label: 'Nama *', key: 'name', placeholder: 'PT. Maju Bersama' },
                { label: 'Email', key: 'email', placeholder: 'admin@maju.com' },
